...
const subscriptionsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post('/sync-from-polar', { preValidation: [requireAuthHook] }, async (req, reply) => {
    try {
      const { organizationId } = (req.body as any) || {};
      // @ts-expect-error from auth hook
      const user = req.user;
      ...
      if (existingSubscription && existingSubscription.status === 'active') {
        return reply.status(400).send({ error: 'Subscription already exists and is active' });
      }

      const polarAccessToken = process.env.POLAR_ACCESS_TOKEN;
      if (!polarAccessToken) {
        return reply.status(500).send({ error: 'Payment system not configured' });
      }
      ...
      const ordersResp = await fetch(`${polarApiBase}/orders?customer_email=${encodeURIComponent(user.email)}`, {
        headers: { Authorization: `Bearer ${polarAccessToken}` },
      });

      if (!ordersResp.ok) {
        app.log.error('Failed to fetch orders from Polar', await ordersResp.text());
        return reply.status(500).send({ error: 'Failed to fetch subscriptions from Polar' });
      }

      const orders = await ordersResp.json();
      app.log.info({ orderCount: orders.items?.length || 0 }, 'Fetched orders from Polar');
      ...
      if (orders.items && Array.isArray(orders.items)) {
        for (const order of orders.items) {
          ...
          if (metadata.organizationId === organizationId || metadata.organization_id === organizationId) {
            matchedOrder = order;
            break;
          }
        }
      ...
      if (!matchedOrder) {
        ...
        const subscriptionsResp = await fetch(`${polarApiBase}/subscriptions?customer_email=${encodeURIComponent(user.email)}`, {
          headers: { Authorization: `Bearer ${polarAccessToken}` },
        });

        if (subscriptionsResp.ok) {
          const subscriptions = await subscriptionsResp.json();
      ...
      if (!matchedOrder) {
        return reply.status(404).send({
          error: 'No subscription found in Polar for this organization. Please check Polar dashboard or contact support.',
        });
      }

      app.log.info({ orderId: matchedOrder.id, status: matchedOrder.status }, 'Found matching order in Polar');

      // Extract metadata
      const metadata = matchedOrder.metadata || matchedOrder.customer?.metadata || {};
      const extractedOrgId = metadata.organizationId || metadata.organization_id || organizationId;
      const extractedUserId = metadata.userId || metadata.user_id || user.id;

      if (extractedOrgId !== organizationId) {
        return reply.status(400).send({ error: 'Organization ID mismatch in Polar subscription metadata' });
      }

      // Process the order similar to webhook handler
      const productId = matchedOrder.product_id || matchedOrder.product?.id || process.env.POLAR_PRODUCT_ID!;
      
      // Ensure product exists in database
      let product = await prisma.product.findUnique({ where: { polarId: productId } });
      if (!product) {
        product = await prisma.product.create({
          data: {
            ...
            priceCents: matchedOrder.price_amount || matchedOrder.amount || 1000,
      ...
      const customer = matchedOrder.customer || {};

      // Determine if payment was successful - check multiple status indicators
      const orderStatus = matchedOrder.status?.toLowerCase() || '';
      const paymentStatus = matchedOrder.payment_status?.toLowerCase() || matchedOrder.payment?.status?.toLowerCase() || '';
      const isPaid = 
      ...
      const paymentId = matchedOrder.payment_id || matchedOrder.payment?.id || matchedOrder.id;
      const paymentAmount = matchedOrder.amount_paid || matchedOrder.amount || matchedOrder.price_amount || product.priceCents;
      ...
      if (paymentId) {
        ...
        if (!existingPayment) {
          ...
          app.log.info({ paymentId, amount: paymentAmount }, 'Payment record created during sync');
        }
      } else {
        ...
        if (payments.length === 0 && paymentAmount > 0) {
          ...
          app.log.info({ amount: paymentAmount }, 'Payment record created without Polar payment ID');
        }
      }
      ...
      app.log.info({ subscriptionId: subscription.id, organizationId }, 'Subscription synced from Polar');
      ...
      return reply.status(500).send({ error: 'Failed to sync subscription from Polar' });
    }
  });
};
