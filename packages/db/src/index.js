...
const isAccelerateOrDataProxyUrl = accelerateUrl?.startsWith("prisma://") ||
...
const isDirectUrlActuallyAccelerate = directUrl?.startsWith("prisma://") ||
...
const shouldUseDirectUrl = !isProductionEnv &&
    !!directUrl &&
    !isAccelerateOrDataProxyUrl &&
    !isDirectUrlActuallyAccelerate;
const databaseUrl = shouldUseDirectUrl ? directUrl : accelerateUrl;
if (!databaseUrl) {
    throw new Error("Prisma client failed to initialize: set DATABASE_URL (production) or DIRECT_URL (development).");
}
console.log(`🔍 DB Package - selected ${shouldUseDirectUrl ? "DIRECT_URL" : "DATABASE_URL"}: ${databaseUrl.substring(0, 50)}...`);
        ...
        user: {
            async create({ args, query }) {
                ...
                if (args.data.role && typeof args.data.role === "string") {
                    ...
                    console.log(`🔄 Role mapping: "${originalRole}" → "${args.data.role}"`);
                }
                return query(args);
            },
...
console.log(`📊 Prisma Client initialized with ${isAccelerateUrl ? "Accelerate" : "direct connection"} and role mapping extension`);
